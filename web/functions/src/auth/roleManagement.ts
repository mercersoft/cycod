...
const VALID_ROLES = ['user', 'moderator', 'admin', 'super-admin'] as const;
...
export const setUserRole = functions.https.onCall(async (data, context) => {
  ...
  const { uid, role } = data as { uid?: string; role?: ValidRole };
  if (!uid || !role) {
  ...
  try {
    ...
    if (role === 'admin' || role === 'super-admin') customClaims.admin = true;
    if (role === 'moderator' || role === 'admin' || role === 'super-admin') customClaims.moderator = true;
...
export const getAllUsers = functions.https.onCall(async (data, context) => {
  ...
  const { pageSize = 50, pageToken } = (data || {}) as { pageSize?: number; pageToken?: string };
