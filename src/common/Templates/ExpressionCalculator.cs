...
{
    public CalcException(string message, string expression, int position) : base(message)
    {
        Position = position;
...
public class ExpressionCalculator
{
    ...
    public ExpressionCalculator()
    {
        ...
        AddFunction(new Function("EQUALS", Equals));
        ...
        while (true)
        {
            ...
            else if (_tokenType == TokenType.BitwiseOr)
            {
                ...
                if (value >= long.MinValue && value <= long.MaxValue)
                {
                    ...
                    {
                        throw new CalcException("Result to large. Bitwise operation conversion impossible.", _expression, _position);
                    }
                }
                else
                {
                    throw new CalcException("Result to large. Bitwise operation impossible.", _expression, _position);
                }
            }
            else
            {
    ...
    private dynamic Factor()
    {
        ...
        else if (_tokenType == TokenType.Identifier)
        {
            ...
            {
                throw new CalcException("Undefined symbol", _expression, _position);
            }
        }
        ...
        {
            throw new CalcException("Expected number, function, or constant", _expression, _position);
        }
        return value;
    }
    ...
    private void NextToken()
    {
        ...
        else
        {
            ...
            else if (_expression[_position] == '"')
            {
                ...
                if (_nextPosition >= _expression.Length)
                {
                    throw new CalcException("Expected closing quote", _expression, _position);
                }
    ...
    private double Exact(double d)
    {
        ...
        throw new CalcException("Result to large. Conversion impossible.", _expression, _position);
    }
    ...
    private dynamic Atan2()
    {
        ...
        {
            throw new CalcException("expected comma", _expression, _position);
        }
        NextToken();
    ...
    public string StrFromDRadix(double d, Radix radix)
    {
        ...
        {
            throw new CalcException("Result to large. Radix conversion impossible.", _expression, _position);
        }
    }
