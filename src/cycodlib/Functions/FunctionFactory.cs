...
{
    /// <summary>
    /// Factory for creating and managing AI functions with logging abstraction
    /// </summary>
    public class FunctionFactory
    {
        ...
        public FunctionFactory(ILogger logger)
        {
            _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        }

        public FunctionFactory(ILogger logger, Assembly assembly) : this(logger)
        {
            AddFunctions(assembly);
        }

        public FunctionFactory(ILogger logger, Type type1, params Type[] types) : this(logger)
        {
            AddFunctions(type1, types);
        }

        public FunctionFactory(ILogger logger, IEnumerable<Type> types) : this(logger)
        {
            AddFunctions(types);
        }
        ...
        {
            const int shortDescriptionMaxCch = 50;
            var shortDescription = aiFunction.Description.Length > shortDescriptionMaxCch
                ? aiFunction.Description.Substring(0, shortDescriptionMaxCch) + "..."
                : aiFunction.Description;

            _logger.WriteDebug($"Adding function '{aiFunction.Name}' - {shortDescription}");
            _functions.TryAdd(aiFunction.Name, (aiFunction, method, instance, readOnly));
            _logger.WriteDebug($"Count of functions in factory: {_functions.Count}");
        }
        ...
        {
            var newFactory = new FunctionFactory(a._logger);
        ...
        private string? TryCallFunction(MethodInfo methodInfo, AIFunction function, string argumentsAsJson, object? instance)
        {
            try
            {
            ...
            {
                _logger.WriteDebug($"Error calling function '{function.Name}': {ex.Message}");
